...
class Gitlab extends Api {
    constructor(options) {
        ...
        this.ticketProjects = new Map;
    }

    addTicketProject(ticket, project) {
        let ticketSet = this.ticketProjects.get(ticket);
        if (!ticketSet)  {
            ticketSet = new Set;
            this.ticketProjects.set(ticket, ticketSet);
        }
        ticketSet.add(project);
        return ticketSet;
    }

    getTicketProjects(ticket) {
        return this.ticketProjects[ticket] || new Set;
    }
    ...
    getMergeTickets(project, merge, body, cb) {
        ...
        this.getMergeCommits(project, merge.sha, body.commits, (err, commits) => {
            ...
            _.each(commits, commit => {
                ...
                _.each(ticketIds, ticketId => {
                    let ticketItem = addTicketInfo(ticketId);
                    ticketItem.projects = Array.from(this.addTicketProject(ticketId, project));
                });
