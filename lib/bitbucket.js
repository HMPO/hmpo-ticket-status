    ...
    getReleaseTags(project, count, cb) {
        this.getTags({ project, count }, (err, body) => {
            ...
            _.each(body.values, tag => {
                ...
                let releaseNumber = tag.displayId && tag.displayId.match(this.config.releaseMatching);
            ...
            async.eachLimit(hashes, this.config.concurrent, (sha, done) => {
                this.getBuilds({ sha }, (err, body) => {
                    ...
                    _.each(body.values, build => {
                        ...
                        releaseTags[sha].builds.push({
                            ...
                            link,
                            promoteProjectLink,
                            result: (build.state === 'SUCCESSFUL') ? 'SUCCESS' : 'FAIL',
                ...
                _.each(body.values, tag => {
                    ...
                    if (!release) return;
                    release.builds.sort((a, b) => b.id - a.id);
        ...
        this.getCommits({ project, count }, (err, body) => {
            ...
            _.each(body.values, commit => {
                ...
                if (commit.parents && commit.parents.length === 2) {
                    let merge = commit.message.match(/(p|P)ull request #(\d+):? (.|\n)* to master\n/);
                    let mergeId = merge && Number(merge[2]);
                    merges.push({
                        id: mergeId,
                        sha: commit.id,
                        releaseId: latestRelease,
                        link: mergeId && buildString('{mergeLink}', _.extend({project, mergeId}, this.config))
                    });
                    return;
                }
