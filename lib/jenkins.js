...
class Jenkins extends Api {
    constructor(options, name) {
        super({
            ...
            ttl: 10000,
            projectFormat: options.projectFormat || '{project:alpha}',
    ...
    getBuildPromotions(project, cb) {
        this.getPromotions({ project }, (err, body) => {
            ...
            _.each(body.processes, env => {
                ...
                if (promotion && env.name && promotion.target && promotion.target.number) {
                    data.promotions[env.name] = {
                        buildId: Number(promotion.target.number),
                        env: env.name,
                        envClass: this.config.environments[env.name] || env.name,
