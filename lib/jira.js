...
class Jira extends Api {
    constructor(options, name) {
        super({
            api: name || 'jira',
            engineMatch: options.ticketMatch,
            auth: options.privateToken ? undefined : options.credentials,
            headers: options.privateToken ? { 'Authorization': 'Bearer ' + options.privateToken } : undefined,
